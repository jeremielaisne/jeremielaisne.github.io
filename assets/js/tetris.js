...
var Ecouteurs = function(p){
    ...
    this.annulEvent = function (){
        document.removeEventListener('keydown', this.handleEvent, false);
    }
    this.activEvent = function (){
...
function main(g, j, booleen)
{
    ...
    {
        var e;
        var pause = document.getElementById('b_pause');
        var restart = document.getElementById('b_restart');
        var focus = document.getElementById('focus_secours');
        document.getElementById('b_focus').focus();
        ...
        function token(nb, g)
        {
            j.miniature_reset();
            j.verifLignes(g);
            j.getListeNb(nb).init()?(j.getListeNb(nb).ajoutGrille(),j.setNiveau(j.getScore()),piece = j.getListeNb(nb),e = new Ecouteurs(j.getListeNb(nb))):(j.getTermine(),j.getListeNb(nb).ajoutGrille(),j.setTermine(true),clearInterval(TmpGame),clearInterval(DownGame),pause.classList.add('d-none'),document.getElementById('b_start').hidden = false);
        ...
        pause.onclick = function(){clearInterval(DownGame);pause.classList.add('d-none');restart.classList.remove('d-none');e.annulEvent();};
        focus.onclick = function(){document.getElementById('b_focus').focus();};
        var DownGame = setInterval(function(){j.getListeNb(nb).down()}, j.getNiveauScore());
        restart.onclick = function(){DownGame = setInterval(function(){j.getListeNb(nb).down()}, j.getNiveauScore());pause.classList.remove('d-none');restart.classList.add('d-none');document.getElementById('b_focus').focus();e.activEvent();};
...
window.onload = function(){
    ...
    clic.onclick = function(){var self=this;setTimeout(function(){self.hidden=true;},0);txt.innerHTML='<p class="font-weight-bold" id="aff_resultat">Il faut utiliser les 4 touches directionnelles du clavier.</p>';main(g, j, false);return false;}
};